...
import { getMerchants, addMerchant, deleteMerchant, Merchant } from '../db/queries';
...
export default function SettingsMerchantsScreen() {
  ...
  const load = useCallback(async () => {
    ...
    setMerchants(await getMerchants(currentHouse.id));
  }, [currentHouse]);
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    try {
      await addMerchant(currentHouse!.id, name);
      setNewName('');
  ...
  const handleDelete = (m: Merchant) => {
    showAlert('Remove Merchant', `Remove "${m.name}" from your list?`, [
        ...
        <View style={s.addSection}>
          ...
          <View style={[s.addRow, focused && s.addRowFocused]}>
            ...
            <TextInput
              ...
              placeholder="e.g. Amazon, Walmart, Netflix"
        ...
        <ScrollView contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
          ...
          {merchants.length === 0 ? (
            ...
            <View style={s.listCard}>
              {merchants.map((m, i) => (
                <View key={m.id} style={[s.row, i < merchants.length - 1 && s.rowBorder]}>
